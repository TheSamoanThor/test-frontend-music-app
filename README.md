...
1. Откройте `index.html` в современном браузере (Chrome, Edge, Opera рекомендуются для полной поддержки File System Access и других API).
...
1. Open `index.html` in a modern browser (Chrome, Edge, Opera recommended for full File System Access support and other APIs).
